...
public class ParametersTests
{
    ...
    [Fact]
    public async Task NoRouteOrBody_AllQueryFields()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
    ...
    [Fact]
    public async Task RouteFields_FilterRouteQueryFields()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
    ...
    [Fact]
    public async Task RouteAndBodyFields_FilterRouteAndBodyQueryFields()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
    ...
    [Fact]
    public async Task CatchAllBody_NoQueryFields()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
    ...
    [Fact]
    public async Task NoBodyComplexType_NestedQueryField()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
    ...
    [Fact]
    public async Task RepeatedStringField_ArrayQueryField()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
        ...
        Assert.Equal(ParameterLocation.Query, operation.Parameters[0].In);
        Assert.Equal("parameterOne", operation.Parameters[0].Name);
        Assert.Equal(JsonSchemaType.Array, operation.Parameters[0].Schema.Type);
        // parameter_one is a "repeated int64", and proto JSON encodes 64-bit integers as
        // strings. The Swashbuckle implementation emitted "integer" here, which was wrong.
        Assert.Equal(JsonSchemaType.String, operation.Parameters[0].Schema.Items.Type);
        Assert.Equal("int64", operation.Parameters[0].Schema.Items.Format);
    }

    [Fact]
    public async Task MultipleRouteParameter_NestedFields_MissingFieldsAreQuery()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
    ...
    [Fact]
    public async Task KnownTypes_AllQueryFields()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
        ...
        Assert.Equal(JsonSchemaType.String, operation.Parameters[0].Schema.Type);
        Assert.Equal(ParameterLocation.Query, operation.Parameters[1].In);
        Assert.Equal("stringValue", operation.Parameters[1].Name);
        Assert.Equal(JsonSchemaType.String, operation.Parameters[1].Schema.Type);
        Assert.Equal(ParameterLocation.Query, operation.Parameters[2].In);
        Assert.Equal("int32Value", operation.Parameters[2].Name);
        Assert.Equal(JsonSchemaType.Integer, operation.Parameters[2].Schema.Type);
        Assert.Equal("int32", operation.Parameters[2].Schema.Format);
    }

    [Fact]
    public async Task Verb()
    {
        // Arrange & Act
        var swagger = await OpenApiTestHelpers.GetOpenApiDocumentAsync<ParametersService>(_testOutputHelper);
